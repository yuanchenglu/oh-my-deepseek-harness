...
| `REL-005` | #16 | PR #59 | Complete — GitHub-only Beta channel selected; PyPI disabled pending authenticated prerequisites |
| `COMPAT-000` | #17 | PR #60 | Complete when this branch confirms the merged compatibility files and G0 Required CI passes |
...
- known compatibility gaps have explicit owners and later Gate blockers.
...
2. preserve `COMPAT-000` findings:
   ...
   - current Context Engine is missing the required `name` property;
