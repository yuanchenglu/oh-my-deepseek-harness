"""Checkpoint Review 数据模型 — Pydantic v2（纯业务层）
...
class CreateCheckpointRequest(BaseModel):
    """创建 Checkpoint 所需的输入数据（非 HTTP 请求模型）"""
    plan_id: str = Field(..., description="Plan 唯一标识")
    plan_steps: List[DictStep] = Field(..., description="Plan 的完整步骤列表")
    completed_step_ids: List[str] = Field(..., description="已完成的步骤 ID 列表")
    unexpected_findings: List[str] = Field(
        default_factory=list,
